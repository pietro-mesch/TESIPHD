...
Finally, it is possible to approach the maximisation of bandwidth as a function of travel
...
\eq[,]{e:slackint}{
\fslackband{j} = \int_{\gini{j}}^{\gini{j}+\gren{j}} l(t) \; \mathrm{d}t
}
where $\gini{j}$ is the beginning time of the through phase at $j$ and $\gren{j}$ its duration, as before.

The integral in \req{e:slack1} clearly formalises the definition of slack bandwidth illustrated in Figure \ref{f:slackdef}.a but it's not practical to compute, and does not provide an explicit form for $l(t)$.
...
\eq[.]{e:slacksum}{
\fslackband{j} = \sum pippapippa
}

Assuming constant speed along the $j^{th}$ corridor section

instead of \req{e:milp6}

which leads to a variation on \req{e:milp7}