\chapter{Modelling, Simulation and Optimisation Tools}
...
In this framework, all resources are accessible to the relevant components, but the separation of responsibilities between them ensures that information is not duplicated \todo{what am I trying to sayyyyyyy???}

\todo{BBAH passata veloce sui componenti per poi passare a TRE. Separation of responsibilities.
}
...
\todo{derive wave propagation speed, conservation, Newell Luke MP, shockwaves}

\todo{OR MAYBE cut back on theory refer to literature and stick with the ESSENTIALS}


\subsection{Node Model} \label{s:nodemodel}
\todo{illustrate node model, specifying how IT is in charge of the implementation of signals}

\fig{htbp}{PIX/undoguy.png}{f:label}{ \todo{figura con le tabelle semaforizzazione} }{width=0.4\textwidth}
...
\todo{explain right away how this applies to our case}
...
\fig{htbp}{PIX/undoguy.png}{f:geneticalgorithm}{The Genetic Algorithm: with each cycle, a new \emph{generation} carrying the most successful traits of the previous one supplants the least successful individuals.}{width=0.4\textwidth}
